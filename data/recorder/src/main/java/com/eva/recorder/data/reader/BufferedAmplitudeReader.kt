...
internal class BufferedAmplitudeReader(
	...
	private val _ampsRange = MutableStateFlow(AmplitudeRange())
	...
	private fun readSampleAmplitude(state: RecorderState): Flow<Int> = flow {
		try {
			...
			Log.e(LOGGER_TAG, "ILLEGAL STATE", err)
		} catch (e: Exception) {
			e.printStackTrace()
		}
	...
	private fun clearBufferAndEmptyFlow(): Flow<List<MicrophoneDataPoint>> {
		if (_buffer.isNotEmpty()) {
			...
			_buffer.clear()
			_ampsRange.update { AmplitudeRange() }
		}
		return flow {
			emit(emptyList())
		}
	...
	private fun List<MicrophoneDataPoint>.normalize(): List<MicrophoneDataPoint> {
		...
		return map { (idx, amp) ->
			val normal = (amp - ampRangeValue.min).toFloat() / range
